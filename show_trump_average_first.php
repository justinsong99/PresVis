<head>
  <title>ShowAverageApproval</title>
</head>
<body>
  ...
  if (!$db) {
    ...
    if(!isset($count) || trim($count) == ''){
      ...
      $result = $db->multi_query("SELECT avg(Approve) FROM (SELECT * FROM 45ApprovalRatings LIMIT $count) as derived;");
      ...
      if (!$result) {
        ...
        if ($result = $db->store_result()) {
          ...
          if (strcmp($columns[0], "ERROR") == 0) {
            ...
            echo "<table border=1>\n";
            echo "<tr><td>Trump's Average Approval Rating</td></tr>\n";
