...
if (!$db) {
 ...
 $result = $db->multi_query("SELECT State FROM (SELECT State, count(*) as count FROM PresWinner WHERE Party NOT LIKE '%democrat%' GROUP BY State HAVING count=11) as derived");
 ...
 if (!$result) {
   ...
   if ($result = $db->store_result()) {
     ...
     if (strcmp($columns[0], "ERROR") == 0) {
       ...
       echo "<table border=1>\n";
       echo "<tr><td font-weight: 'bold'>STATE</td></tr>\n";
