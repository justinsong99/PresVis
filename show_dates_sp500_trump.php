  ...
  if (!$db) {
    ...
    // input to the PHP code. In this case, the input is called 'ID'.
      ...
      if (strcmp($high_or_low, "high") == 0) {
        $result = $db->multi_query("SELECT Date, High FROM sp500 as sp, 45ApprovalRatings as ar WHERE ar.Day=sp.Date AND ar.approve IN (SELECT min(approve) FROM 45ApprovalRatings)");
      } else {
        $result = $db->multi_query("SELECT Date, Low FROM sp500 as sp, 45ApprovalRatings as ar WHERE ar.Day=sp.Date AND ar.approve IN (SELECT min(approve) FROM 45ApprovalRatings)");
      }
